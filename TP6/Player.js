...
window.addEventListener("keydown", (e) => {
  ...
  // Déclenche l'attaque si la touche E est pressée et que l'attaque n'est pas en cooldown
  if (e.code === configTouches.ATTAQUE && player.canAttack) {
    player.ATKing = true;
    player.canAttack = false;
    player.cooldownTimer = Date.now();
    // Réactive l'attaque après le délai de cooldown
    setTimeout(() => {
      if (player) player.canAttack = true;
    }, player.attackCooldown);
  }

  // Touche de test : inflige 20 dégâts au joueur
  ...
  // Touche de test : tue le joueur instantanément
...
// Gère le déplacement du joueur selon les touches pressées
...
function gameLoop() {
  ...
  if (player) {
    ...
    // Supprime le joueur du jeu s'il doit être retiré
    if (player.shouldBeRemoved) {
      player = null;
      console.log("Joueur retiré de la scène.");
    }
  }
  // Relance la boucle au prochain rafraîchissement d'écran
